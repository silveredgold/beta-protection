<template>
    <n-card title="Video Censoring Preferences" size="small">
        <n-thing title="Censor Mode" v-if="loaded">
            <n-radio-group
                v-model:value="prefs.videoCensorMode"
                name="left-size"
                size="small"
        ...
        <template
            #footer
        >Block will hide all videos from view, Blur will display videos but blur the video, Allow leaves videos untouched.</template>
        <n-thing
            title="Censor Level"
            description="Higher levels cause more blur"
            v-if="loaded && !compact && prefs.videoCensorMode === 'Blur'"
        >
            <n-slider
                :min="1"
                :max="10"
